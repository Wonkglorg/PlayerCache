...
public class PlayerDB extends SqliteDatabase<FileDataSource>{
		...
		RegionScheduler.getInstance().runAsync(() -> {
			...
			try{
				...
				try(var deactivateStmt = connection.prepareStatement("UPDATE players SET active = 0 WHERE uuid = ? AND name != ? AND active = 1");
					var upsertStmt = connection.prepareStatement("INSERT INTO players (uuid, name, first_joined, last_seen, is_bedrock, active) " +
																 "VALUES (?, ?, ?, ?, ?, 1) " +
	...
	public void addBatchProfile(List<OfflinePlayer> profiles) {
		if(profiles.isEmpty()){
			return;
		}
		...
		try{
			...
			try(var deactivateStmt = connection.prepareStatement("UPDATE players SET active = 0 WHERE uuid = ? AND name != ? AND active = 1");
				var upsertStmt = connection.prepareStatement("INSERT INTO players (uuid, name, first_joined, last_seen, is_bedrock, active) " +
															 ...
															 "is_bedrock = excluded.is_bedrock, " +
															 "active = 1")){
				for(OfflinePlayer profile : profiles){
					UUID uniqueId = profile.getUniqueId();
					deactivateStmt.setString(1, uniqueId.toString());
					deactivateStmt.setString(2, profile.getName());
					deactivateStmt.addBatch();
					
					upsertStmt.setString(1, uniqueId.toString());
					upsertStmt.setString(2, profile.getName());
					upsertStmt.setLong(3, profile.getFirstPlayed());
					upsertStmt.setLong(4, profile.getLastSeen());
					upsertStmt.setBoolean(5, PlayerProfile.isBedrock(uniqueId));
					upsertStmt.addBatch();
				}
				...
				PluginLogger.error("Error during insert", e);
				connection.rollback();
				throw e;
			} finally{
				connection.setAutoCommit(true);
